...
def train(model, train_loader, val_loader, device, epochs=10):
    ...
    # Plot & Save
    torch.save(model.state_dict(), "nocrop_model_shape_fusion.pt")
    print("✅ Model saved: nocrop_model_shape_fusion.pt")
...
def plot_results(losses, accs):
    ...
    plt.savefig("nocrop_training_curve.png")
    print("📈 Saved: nocrop_training_curve.png")
...
def save_report(y_true, y_pred):
    ...
    plt.savefig("nocrop_confusion_matrix.png")
    print("📊 Saved: nocrop_confusion_matrix.png")
    with open("nocrop_val_report.txt", "w") as f:
        f.write(classification_report(y_true, y_pred, target_names=labels))
    print("📄 Saved: nocrop_val_report.txt")

# === Entry ===
if __name__ == "__main__":
    csv = "nocrop_image_dataset_with_shapes.csv"
    df = pd.read_csv(csv)
    df["disaster"] = df["file"].apply(lambda x: x.split("_")[0])  # 災害名を抽出
    ...
    train_df.to_csv("nocrop_train_event_split.csv", index=False)
    val_df.to_csv("nocrop_val_event_split.csv", index=False)

    train_loader = DataLoader(ShapeFusionDataset("nocrop_train_event_split.csv"), batch_size=32, shuffle=True)
    val_loader = DataLoader(ShapeFusionDataset("nocrop_val_event_split.csv"), batch_size=32)
